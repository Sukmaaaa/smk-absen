...
@section('dashboard_url', 'Tambah Peran')
    ...
    <form action="{{ route('role.store') }}" method="POST" class="container">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <x-adminlte-input type="text" name="name" placeholder="admin" title="Isi nama role">
                    </x-adminlte-input>
                    ...
                    @foreach($permissions as $key => $permission)
                        ...
                        <div class="icheck-primary col-md-3">
                            <input type="checkbox" name="permission[]" id="{{ $permission->name }}" value="{{ $permission->name }}" title="Pilih izin">
                            <label for="{{ $permission->name }}" title="Pilih izin">{{ $permission->name }}</label>
                        </div>
